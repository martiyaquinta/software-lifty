...
if (!existsSync(envPath)) {
  ...
  await Bun.write(
    envPath,
    [
      '# Generated by `bun run setup`. DATABASE_URL is not needed in dev:',
      '# the backend defaults to the docker-compose.dev.yml Postgres (5433/lifty).',
      '# WARNING: if you do set DATABASE_URL here, never point it at a DB you',
      '# care about and then run tests — tests truncate every table.',
      `PORT=${apiPort}`,
      `JWT_SECRET=${secret}`,
